...
DataRec GetDataRec(string Namestr, Boolean Train)
/*      ----------  */
{
    Attribute	Att;
    char* Name;
	...
	ForEach(Att, 1, MaxAtt)
	{
	    ...
	    if ( ! FirstValue && !  getSubStr(Namestr,Name)) 
	    {
...
/*	 -------  */
{
	...
	switch ( ReadProp(&Delim) )
	{
		...
		CheckActiveSpace(RS->SNRules, GCEnv[gDepth]);
		break;
...
/*	-------------  */
{
	...
	Best = ( RULES ? RuleClassify(Case, RuleSet[t], E) :
			 TreeClassify(Case, Pruned[gDepth][t], E) );
...
ClassNo SelectClass(ClassNo dDefault, Boolean UseCosts, double *Prob)
/*      -----------  */
{
    ClassNo	c, BestClass;
...
/*   -----------  */
{
	...
	FreeUnlessNil(GCEnv[gDepth]->Active);
	FreeUnlessNil(GCEnv[gDepth]->RulesUsed);
	FreeUnlessNil(GCEnv[gDepth]->MostSpec);
    }
    else
    {
	ForEach(Trial, 0, TRIALS-1)
    ...
    free(GCEnv[gDepth]->ClassWt);
    free(GCEnv[gDepth]->Vote);
    free(GCEnv[gDepth]);
}
